...
const unordered_set<uint32_t> ReturnInvalidCodepoints() {
  return {// 0x0001 to 0x0008 - control characters
          ...
          0xb, 0xfffe, 0xffff, 0x1fffe, 0x1ffff, 0x2fffe, 0x2ffff, 0x3fffe,
...
// define the equality and inequality operators between HTMLEscapeCodepoint
// objects
bool operator==(const HTMLEscapeCodepoint &lhs,
                const HTMLEscapeCodepoint &rhs) {
  return lhs.codepoints[0] == rhs.codepoints[0] &&
         lhs.codepoints[1] == rhs.codepoints[1];
}
...
// This maps HTML5 named character references to the equivalent Unicode
// character(s). copied from https://html.spec.whatwg.org/entities.json
