...
struct UnescapeBindData : public FunctionData {
  ...
  UnescapeBindData()
      : html5_names(HTML5NameCharrefs()),
        invalid_codepoints(ReturnInvalidCodepoints()),
  ...
  bool Equals(const FunctionData &other_p) const override {
    ...
    bool equal_sizes =
        (html5_names.mapped_strings.size() ==
             other.html5_names.mapped_strings.size() &&
         invalid_codepoints.size() == other.invalid_codepoints.size() &&
         special_invalid_charrefs.size() ==
             other.special_invalid_charrefs.size());
    return (equal_sizes &&
            (html5_names.mapped_strings == other.html5_names.mapped_strings &&
...
struct MatchLengthCounter {
  ...
  static void Operation(const char *input_data, idx_t start, idx_t count,
                        RESULT_TYPE &result) {
    result += count;
  }
...
struct Matcher {
  template <class OP>
  static void Decode(string_t &input, UnescapeBindData &info,
                     typename OP::RESULT_TYPE &result) {
    auto input_data = input.GetData();
    auto input_size = input.GetSize();
    uint32_t num =
        0; // the interpretation of the input string to the actual number
    ...
    for (idx_t i = 0; i < input_size; ++i) {
      if (type != MatchType::Unknown) {
        if (input_data[i] == '&') { // the beginning of a new match
          ...
          if (!is_valid) {
            if (start == i) {
              DecodeMatch<OP>(MatchType::Unknown, input_data,
                              start - idx_t(type), i, 0, result, info);
            } else {
              DecodeMatch<OP>(type, input_data, start, i - 1, num, result,
                              info);
    ...
    if (type != MatchType::Unknown) {
      DecodeMatch<OP>(type, input_data, start, input_size - 1, num, result,
                      info);
    }
  }
  ...
  template <class OP>
  static void Append(const char *input_data, idx_t start, idx_t end,
                     typename OP::RESULT_TYPE &result) {
    OP::Operation(input_data, start, end - start + 1, result);
  }

  template <class OP>
  static void DecodeMatch(MatchType type, const char *input_data, idx_t start,
                          idx_t end, uint32_t num,
                          typename OP::RESULT_TYPE &result,
                          UnescapeBindData &info, bool semicolon = false) {
    if (type == MatchType::Unknown) {
  ...
  template <class OP>
  static void AppendUnicodeMatch(uint32_t num, typename OP::RESULT_TYPE &result,
    ...
    if (bind_data.invalid_codepoints.find(num) !=
        bind_data.invalid_codepoints.end()) {
      // skip this character
      return;
    }
    int utf8_sz = 0;
  ...
  // decode character reference - looks for the biggest valid charref in the
  // match
  template <class OP>
  static void AppendCharRefMatch(const char *input_data, idx_t start, idx_t end,
                                 typename OP::RESULT_TYPE &result,
                                 UnescapeBindData &bind_data) {
    idx_t sz = end - start + 1;
    if (sz < 33) { // the biggest valid character reference has 32 characters
      for (; sz >= 1; --sz) {
        auto it = bind_data.html5_names.mapped_strings.find(
            string_t(input_data + start, sz));
        if (it != bind_data.html5_names.mapped_strings.end()) {
          char glyph[8] = {};
          // encode first codepoint
          int utf8_sz = 0;
          if (!Utf8Proc::CodepointToUtf8(it->second.codepoints[0], utf8_sz,
                                         glyph)) {
            throw InternalException(
                "Cannot encode to utf8 the first codepoint of &%s",
                it->first.GetString());
          }
          // encode second codepoint (if it exists)
          int utf8_sz2 = 0;
          if (it->second.codepoints[1] != 0 &&
              !Utf8Proc::CodepointToUtf8(it->second.codepoints[1], utf8_sz2,
                                         glyph + utf8_sz)) {
            throw InternalException(
                "Cannot encode to utf8 the second codepoint of &%s",
                it->first.GetString());
          }
          Append<OP>(glyph, 0, utf8_sz + utf8_sz2 - 1, result);
          Append<OP>(input_data, start + sz, end,
                     result); // append the rest of the match
          return;
        }
      }
    }
  ...
  static bool CheckDecDigit(const char *input_data, idx_t pos, uint32_t &num) {
    ...
    if (-1 < dec_digit && dec_digit < 10) {
      if (num * 10 <
          (UINT32_MAX - dec_digit) / 10) { // prevent UINT32_MAX overflowing
        num *= 10;
        num += dec_digit;
      }
      return true;
  ...
  static bool CheckHexDigit(const char *input_data, idx_t pos, uint32_t &num) {
    ...
    if (hex_digit > -1) {
      if (num * 16 <
          (UINT32_MAX - hex_digit) / 16) { // prevent UINT32_MAX overflowing
        num *= 16;
        num += hex_digit;
      }
      return true;
...
static string_t EscapeInputStr(string_t &input_str, bool input_quote,
                               Vector &result) {
  constexpr idx_t QUOTE_SZ = 1;
  constexpr idx_t AMPERSAND_SZ = 5;
  constexpr idx_t ANGLE_BRACKET_SZ = 4;
  constexpr idx_t TRANSLATED_QUOTE_SZ =
      6; // e.g. \" is translated to &quot;, \' is translated to &#x27;
...
void INetFunctions::Escape(DataChunk &args, ExpressionState &state,
                           Vector &result) {
  if (args.ColumnCount() == 1) {
    UnaryExecutor::Execute<string_t, string_t>(
        args.data[0], result, args.size(), [&](string_t input_str) {
    ...
    BinaryExecutor::Execute<string_t, bool, string_t>(
        ...
        [&](string_t input_str, bool input_quote) {
          return EscapeInputStr(input_str, input_quote, result);
        });
  }
}
...
ScalarFunction InetExtension::GetUnescapeFunction() {
  return ScalarFunction("html_unescape", {LogicalType::VARCHAR},
                        LogicalType::VARCHAR, INetFunctions::Unescape,
                        UnescapeBind);
}
