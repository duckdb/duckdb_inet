...
bool INetFunctions::CastVarcharToINET(Vector &source, Vector &result,
...
bool INetFunctions::CastINETToVarchar(Vector &source, Vector &result,
                                      idx_t count, CastParameters &parameters) {
  GenericExecutor::ExecuteUnary<INET_TYPE, PrimitiveType<string_t>>(
      source, result, count, [&](INET_TYPE input) {
...
void INetFunctions::Host(DataChunk &args, ExpressionState &state, Vector &result) {
  GenericExecutor::ExecuteUnary<INET_TYPE, PrimitiveType<string_t>>(
      args.data[0], result, args.size(), [&](INET_TYPE input) {
        ...
        auto mask = inetType == IPAddressType::IP_ADDRESS_V4
...
void INetFunctions::Netmask(DataChunk &args, ExpressionState &state,
                            Vector &result) {
  GenericExecutor::ExecuteUnary<INET_TYPE, INET_TYPE>(
      args.data[0], result, args.size(), [&](INET_TYPE input) {
...
void INetFunctions::Network(DataChunk &args, ExpressionState &state,
                            Vector &result) {
  GenericExecutor::ExecuteUnary<INET_TYPE, INET_TYPE>(
      args.data[0], result, args.size(), [&](INET_TYPE input) {
        IPAddressType type = IPAddressType(input.a_val);
...
void INetFunctions::Broadcast(DataChunk &args, ExpressionState &state,
                              Vector &result) {
  GenericExecutor::ExecuteUnary<INET_TYPE, INET_TYPE>(
      args.data[0], result, args.size(), [&](INET_TYPE input) {
        IPAddressType type = IPAddressType(input.a_val);
...
static INET_TYPE AddImplementation(INET_TYPE ip, hugeint_t val) {
  ...
  if (addr_type == IPAddressType::IP_ADDRESS_V4 &&
      address_out >= (uhugeint_t(0xffffffff))) {
    throw OutOfRangeException(
        "Cannot add %s to %s.", val,
        IPAddress(addr_type, address_in, ip.c_val).ToString());
  }
...
void INetFunctions::ContainsLeft(DataChunk &args, ExpressionState &state, Vector &result) {
  GenericExecutor::ExecuteBinary<INET_TYPE, INET_TYPE, PrimitiveType<bool>>(
      ...
      [&](INET_TYPE ip_left, INET_TYPE ip_right) {
        return ContainsImplementation(ip_left, ip_right);
      });
}

void INetFunctions::ContainsRight(DataChunk &args, ExpressionState &state,
                                  Vector &result) {
  GenericExecutor::ExecuteBinary<INET_TYPE, INET_TYPE, PrimitiveType<bool>>(
      args.data[0], args.data[1], result, args.size(),
      [&](INET_TYPE ip_left, INET_TYPE ip_right) {
