...
constexpr static const uhugeint_t IPV6_NETWORK_MASK =
    uhugeint_t(0xffffffffffffffff, 0xffffffffffffffff);

IPAddress::IPAddress() : type(IPAddressType::IP_ADDRESS_INVALID) {}

IPAddress::IPAddress(IPAddressType type, uhugeint_t address, uint16_t mask)
...
static bool IPAddressError(string_t input, CastParameters &parameters,
                           string error) {
  string e = "Failed to convert string \"" + input.GetString() +
             "\" to inet: " + error;
...
static bool TryParseIPv4(string_t input, IPAddress &result,
...
parse_number:
  ...
  uint8_t number;
  if (!TryCast::Operation<string_t, uint8_t>(string_t(data + start, c - start),
                                             number)) {
    return IPAddressError(input, parameters,
...
parse_mask:
  ...
  if (!TryCast::Operation<string_t, uint8_t>(string_t(data + start, c - start),
                                             mask)) {
    return IPAddressError(input, parameters,
                          "Expected a number between 0 and 32");
  }
  if (mask > 32) {
    return IPAddressError(input, parameters,
                          "Expected a number between 0 and 32");
...
static bool TryParseIPv6(string_t input, IPAddress &result,
                         CastParameters &parameters) {
  auto data = input.GetData();
  auto size = input.GetSize();
  idx_t c = 0;
  ...
  while (c < size && parsed_quibble_count < IPAddress::IPV6_NUM_QUIBBLE) {
    ...
    if (c < size && data[c] == '.') {
      // This might be the IPv4 dotted decimal form, but it must occur at the
      // end so find the full length, and confirm only valid characters are
      // present.
      c = start;
      while (c < size &&
             (StringUtil::CharacterIsDigit(data[c]) || data[c] == '.')) {
        ++c;
      }
      ...
      quibbles[parsed_quibble_count++] =
          ipv4.address.lower >> IPAddress::IPV6_QUIBBLE_BITS;
    ...
    if (c < size && data[c] == '/') {
      ...
      uint8_t mask;
      if (!TryCast::Operation<string_t, uint8_t>(
              string_t(&data[start], c - start), mask)) {
        return IPAddressError(input, parameters,
  ...
  if (parsed_quibble_count < IPAddress::IPV6_NUM_QUIBBLE &&
      first_quibble_count == -1) {
    return IPAddressError(input, parameters,
  ...
  // Operate on each half of the 128 bit address directly to make the bit
  // operations much more efficient.
  result.address.upper = 0;
  result.address.lower = 0;

  idx_t output_idx = 0;
  for (int parsed_idx = 0; parsed_idx < parsed_quibble_count;
       ++parsed_idx, ++output_idx) {
    if (parsed_idx == first_quibble_count) {
      ...
      // Advanced the output by the number of missing quibbles, they will be
      // zero.
    ...
    if (is_upper) {
      result.address.upper |= static_cast<uint64_t>(quibbles[parsed_idx])
                              << bitshift;
    } else {
      result.address.lower |= static_cast<uint64_t>(quibbles[parsed_idx])
                              << bitshift;
    }
  }
...
bool IPAddress::TryParse(string_t input, IPAddress &result,
                         CastParameters &parameters) {
  auto data = input.GetData();
  auto size = input.GetSize();
  // Start by detecting whether the string is an IPv4 or IPv6 address, or
  // neither.
  idx_t c = 0;
  while (c < size && StringUtil::CharacterIsHex(data[c])) {
    c++;
...
static string ToStringIPv6(const IPAddress &addr) {
  ...
  for (idx_t i = 0; i < IPAddress::IPV6_NUM_QUIBBLE; ++i) {
    ...
    if (is_upper) {
      quibbles[i] =
          Hugeint::Cast<uint16_t>((addr.address.upper >> bitshift) & 0xFFFF);
    } else {
      quibbles[i] =
          Hugeint::Cast<uint16_t>((addr.address.lower >> bitshift) & 0xFFFF);
    }

    if (quibbles[i] == 0 && this_zero_start == IPAddress::IPV6_NUM_QUIBBLE) {
      this_zero_start = i;
    } else if (quibbles[i] != 0 &&
  ...
  for (idx_t i = 0; i < IPAddress::IPV6_NUM_QUIBBLE; ++i) {
    ...
    if (i < zero_end && i >= zero_start) {
      ...
      // suppress ToStringIPv4 from trying to print a mask value
      result << ToStringIPv4(addr.address & 0xffffffff,
                             IPAddress::IPV4_DEFAULT_MASK);
      break;
    } else {
      result << quibbles[i];
    }
...
IPAddress IPAddress::Netmask() const {
  uhugeint_t mask = this->type == IPAddressType::IP_ADDRESS_V4
                        ? IPV4_NETWORK_MASK
                        : IPV6_NETWORK_MASK;
...
IPAddress IPAddress::Network() const {
  return IPAddress(this->type, (this->address & this->Netmask().address),
                   this->mask);
}

IPAddress IPAddress::Broadcast() const {
  return IPAddress(this->type,
                   (this->Network().address | ~this->Netmask().address),
                   this->mask);
}
